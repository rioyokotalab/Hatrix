...
Hatrix::RowLevelMap transform_bases(int64_t height,
				    Hatrix::RowLevelMap& W, Hatrix::RowLevelMap& A) {
  Hatrix::RowLevelMap P;
  for(int64_t level = height; level > 0; level--) {
    int64_t num_nodes = pow(2, level);
    for(int64_t node = 0; node < num_nodes; node++) {
      if(level == height) { //Leaf
	P.insert(node, level, Hatrix::matmul(W(node, level), A(node, level), true));
      }
      else { //Non-leaf
	int64_t child_level = level + 1;
	int64_t child1 = 2*node;
	...
	Hatrix::Matrix tmp(P(child1, child_level).rows + P(child2, child_level).rows,
			   A(node, level).cols);
	auto tmp_splits = tmp.split(2, 1);
	auto Atransfer_splits = A(node, level).split(2, 1);
	Hatrix::matmul(P(child1, child_level), Atransfer_splits[0], tmp_splits[0]);
	Hatrix::matmul(P(child2, child_level), Atransfer_splits[1], tmp_splits[1]);
	P.insert(node, level, Hatrix::matmul(W(node, level), tmp, true));
      }
    }
  }
  return P;
...
void matmul(Hatrix::HSS& A, Hatrix::HSS& B, Hatrix::HSS& C,
  ...
  Hatrix::RowLevelMap Pca = transform_bases(C.height, C.U, A.U);
  Hatrix::RowLevelMap Pab = transform_bases(C.height, A.V, B.U);
  Hatrix::RowLevelMap Pbc = transform_bases(C.height, B.V, C.V);
...
int main(int argc, char** argv) {
  ...
  int64_t k = argc > 3 ? atoi(argv[3]) : 0;
  int64_t height = 2;
  ...
  Hatrix::HSS A(A_randpts, N, rank, height);
  Hatrix::HSS B(B_randpts, N, rank, height);
  Hatrix::HSS C(C_randpts, N, rank+k, height);
  matmul(A, B, C, 1, 1);

  Hatrix::Matrix A_dense = Hatrix::generate_laplacend_matrix(A_randpts, N, N, 0, 0);
  Hatrix::Matrix B_dense = Hatrix::generate_laplacend_matrix(B_randpts, N, N, 0, 0);
  Hatrix::Matrix C_dense = Hatrix::generate_laplacend_matrix(C_randpts, N, N, 0, 0);
  Hatrix::matmul(A_dense, B_dense, C_dense, false, false, 1, 1);
  
  std::cout <<"N=" <<N <<", rank=" <<rank <<", rank_C=" <<rank+k <<", height=" <<height;
  std::cout <<", construction error=" <<A.construction_rel_error(A_randpts);
  std::cout <<", matmul error=" <<C.rel_error(C_dense) <<"\n";
