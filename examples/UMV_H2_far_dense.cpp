...
namespace Hatrix {
  ...
  void H2::update_row_basis(int64_t row, int64_t level, RowColMap<Matrix>& F, RowMap& r) {
    ...
    for (int64_t j = 0; j < nblocks; ++j) {
      if (F.exists(row, j)) {
        if (row == 1) {
          std::cout << "row fill in: row -> " << row << " j -> " << j << std::endl;
        }
  ...
  void H2::factorize_level(int64_t level, int64_t nblocks, const Domain& domain,
    ...
    for (int block = 0; block < nblocks; ++block) {
          ...
          for (int j = 0; j < nblocks; ++j) {
            if (is_admissible.exists(block, j, level) && is_admissible(block, j, level)) {
              ...
              if (F.exists(block, j)) {
                  ...
                  for (int64_t i = 0; i < nblocks; ++i) {
                    if (is_admissible.exists(i, j, level) && is_admissible(i, j, level)) {
                      ...
                      if (i != block) {
                        ...
                        // if (j == 5) {
                        //   continue;
                        //   std::cout << "//// update S for j = 5; i -> " << i << std::endl;
                        // }
                        S.erase(i, j, level);
                ...
                // t.erase(j);
                if (block == 8) {
                  std::cout << "ERASING FILL IN block -> " << 8 << " j -> " << j << std::endl;
                }
                F.erase(block, j);
              }
              else {
              ...
              // if (j == 5) {
              //   continue;
              //   std::cout << "----- update S for j = 5; block -> " << block << std::endl;
              // }
              S.erase(block, j, level);
              S.insert(block, j, level, std::move(Sbar_block_j));
            }
        ...
        if (found_col_fill_in) {
          for (int i = 0; i < nblocks; ++i) {
            if (is_admissible.exists(i, block, level) && is_admissible(i, block, level)) {
              ...
              if (block == 5) {
                std::cout << "!!!!! update S for block = 5; i -> " << i << std::endl;
              }
      ...
      if (block == 8) {
        for (int j = 0; j < nblocks; ++j) {
          if (F.exists(block, j)) {
            std::cout << "FILL EXISTS block -> " << block << " j -> " << j << std::endl;
          }
        }
      }
      // F.erase_all();
  ...
  H2::calc_geometry_based_admissibility(const Domain& domain) {
    ...
    // is_admissible.insert(0, 5, level, false);
    ...
    // is_admissible.insert(5, 0, level, false);
