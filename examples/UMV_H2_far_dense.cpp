...
namespace Hatrix {
  ...
  void H2::update_row_basis(int64_t row, int64_t level, RowColMap<Matrix>& F, RowMap& r) {
    ...
    for (int64_t j = 0; j < nblocks; ++j) {
      if (F.exists(row, j)) {
        ...
        // row_block = concat(row_block, matmul(Ui, Si), 1);
        ...
        if (F(row, j).rows == block_size && F(row, j).cols == rank)  {
          ...
          row_block = concat(row_block, matmul(F(row, j), V(j, level)), 1);
        }
      }
    }
  ...
  void H2::update_col_basis(int64_t col, int64_t level, RowColMap<Matrix>& F, RowMap& t) {
    ...
    for (int64_t i = 0; i < nblocks; ++i) {
      if (F.exists(i, col)) {
        ...
        // Matrix cpy(F(i, col));
        // std::tie(Ui, Si, Vi) = error_svd(cpy, 1e-9);
        // col_block = concat(col_block, matmul(Si, Vi), 0);
        ...
        if (F(i, col).rows == rank && F(i, col).cols == block_size) {
          ...
          col_block = concat(col_block, matmul(U(i, level), F(i, col), true, false), 0);
        }
      }
    }
    ...
    Matrix t_col = matmul(VNT_col, V(col, level)); //matmul(V(col, level), VNT_col, true, true);
    V.erase(col, level);
  ...
  void H2::factorize_level(int64_t level, int64_t nblocks, const Domain& domain,
    ...
    for (int64_t block = 0; block < nblocks; ++block) {
      ...
      //     for (int64_t j = 0; j < nblocks; ++j) {
      //       if (F.exists(block, j) && F(block,j).rows == block_size &&
  ...
  H2::H2(const Domain& domain, int64_t _N, int64_t _rank, int64_t _nleaf,
    ...
    PV = 1.0 / double(N);// * (1 / pow(10, height));
