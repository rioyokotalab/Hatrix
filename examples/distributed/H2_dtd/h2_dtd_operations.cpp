      ...
      if (exists_and_inadmissible(A, i, block, level) &&
        ...
        if (exists_and_inadmissible(A, i, j, level)) {
          ...
          if (i == j) {
            ...
            int64_t D_j_block_nrows = get_dim(A, domain, j, level);
            int64_t D_j_block_ncols = get_dim(A, domain, block, level);
            ...
            parsec_dtd_insert_task(dtd_tp, task_schurs_complement_2, 70, PARSEC_DEV_CPU,
              ...
              PASSED_BY_REF, parsec_dtd_tile_of(&parsec_D.super, D_i_block_key),
                                   PARSEC_INPUT | dense_arena,
              sizeof(int64_t), &D_j_block_nrows, PARSEC_VALUE,
              sizeof(int64_t), &D_j_block_ncols, PARSEC_VALUE,
  ...
  for (int64_t i = block+1; i < nblocks; ++i) {
    for (int64_t j = 0; j < block; ++j) {
      if (exists_and_inadmissible(A, i, block, level) &&
        ...
        if (exists_and_inadmissible(A, i, j, level)) {
          int64_t D_i_dim = get_dim(A, domain, i, level);
          int64_t D_j_dim = get_dim(A, domain, j, level);
          int64_t D_block_dim = get_dim(A, domain, block, level);
          int64_t A_i_rank = A.ranks(i, level);
          int64_t A_j_rank = A.ranks(j, level);
          int64_t A_block_rank = A.ranks(block, level);
          ...
          parsec_dtd_insert_task(dtd_tp, task_schurs_complement_4, 70, PARSEC_DEV_CPU,
            ...
            sizeof(int64_t), &D_block_dim, PARSEC_VALUE,
            sizeof(int64_t), &A_i_rank, PARSEC_VALUE,
            sizeof(int64_t), &A_j_rank, PARSEC_VALUE,
            sizeof(int64_t), &A_block_rank, PARSEC_VALUE,
            PASSED_BY_REF, parsec_dtd_tile_of(&parsec_D.super, D_i_block_key),
                                 PARSEC_INPUT | dense_arena,
            PASSED_BY_REF, parsec_dtd_tile_of(&parsec_D.super, D_block_j_key),
                                 PARSEC_INPUT | dense_arena,
  ...
  if (parent_level > 0) {
    ...
    parsec_dtd_insert_task(dtd_tp, task_transfer_basis_update, 50, PARSEC_DEV_CPU,
       ...
       PASSED_BY_REF, parsec_dtd_tile_of(&parsec_r.super, r_c2_key),
                           PARSEC_INPUT | S_ARENA,
       PASSED_BY_REF, parsec_dtd_tile_of(&parsec_U.super, U_key),
                           PARSEC_INOUT | U_NON_LEAF_ARENA | PARSEC_AFFINITY,
       PARSEC_DTD_ARG_END);
  }
}
...
update_col_transfer_bases(SymmetricSharedBasisMatrix& A,
  ...
  if (parent_level > 0) {
    ...
    parsec_dtd_insert_task(dtd_tp, task_transfer_basis_update, 50, PARSEC_DEV_CPU,
      ...
      sizeof(int64_t), &rank_c1, PARSEC_VALUE,
      sizeof(int64_t), &rank_c2, PARSEC_VALUE,
      PASSED_BY_REF, parsec_dtd_tile_of(&parsec_t.super, t_c1_key),
                           PARSEC_INPUT | S_ARENA,
      PASSED_BY_REF, parsec_dtd_tile_of(&parsec_t.super, t_c2_key),
                           PARSEC_INPUT | S_ARENA,
      PASSED_BY_REF, parsec_dtd_tile_of(&parsec_U.super, U_key),
                           PARSEC_INOUT | U_NON_LEAF_ARENA | PARSEC_AFFINITY,
      PARSEC_DTD_ARG_END);
