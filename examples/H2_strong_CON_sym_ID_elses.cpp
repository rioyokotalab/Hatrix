...
#pragma omp parallel for
    for (int64_t i = 0; i < A.level_nblocks[level]; i++) {
        for (int64_t j : A.inadmissible_cols(i, level)) {
            Matrix Dij = generate_p2p_matrix(domain, domain.get_cell_index(i, level), domain.get_cell_index(j, level));
#pragma omp critical
...
int main(int argc, char** argv) {
    ...
    const double overall_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(stop_overall - start_overall).count();
