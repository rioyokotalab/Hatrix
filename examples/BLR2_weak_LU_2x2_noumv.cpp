...
void factorize_2x2_BLR(BLR_2x2& A, BLR_2x2& L, BLR_2x2& U) {
  ...
  // TRSMs
  Hatrix::solve_triangular(L.D(0, 0), U.U(0), Hatrix::Left, Hatrix::Lower,
                           true);
  Hatrix::solve_triangular(U.D(0, 0), L.V(0), Hatrix::Right, Hatrix::Upper,
                           false);
  ...
  double top_left_diff = Hatrix::norm(L.D(0, 0) * U.D(0, 0) - A_check.D(0, 0));
  std::cout << "Top left error: " << top_left_diff << "\n";

  double top_right_diff =
      Hatrix::norm(L.D(0, 0) * U.U(0) * U.S(0, 1) * U.V(1) - A_check.D(0, 1));
  std::cout << "Top right error: " << top_right_diff << "\n";

  double bottom_left_diff =
      Hatrix::norm(L.U(1) * L.S(1, 0) * L.V(0) * U.D(0, 0) - A_check.D(1, 0));
  std::cout << "Bottom left error: " << bottom_left_diff << "\n";

  double bottom_right =
      Hatrix::norm(L.U(1) * L.S(1, 0) * L.V(0) * U.U(0) * U.S(0, 1) * U.V(1) +
                   L.D(1, 1) * U.D(1, 1) - A_check.D(1, 1));
  std::cout << "Bottom right error: " << bottom_right << "\n\n";
}

void solve_2x2_BLR(const BLR_2x2& L, const BLR_2x2& U, Hatrix::Matrix& x0,
                   Hatrix::Matrix& x1, const Hatrix::Matrix& b0,
                   const Hatrix::Matrix& b1) {
