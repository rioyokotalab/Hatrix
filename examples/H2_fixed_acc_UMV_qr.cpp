...
H2::generate_column_cluster_bases(int64_t block, int64_t block_size,
  ...
  Matrix Vi, R;
  std::tie(Vi, R) = truncated_pivoted_qr(block_column_T, accuracy);
  Matrix Si_T(R.rows, R.rows);
  Matrix Ui_T(R.rows, R.cols);
  rq(R, Si_T, Ui_T);

  int64_t rank = Vi.cols;
  min_rank = std::min(min_rank, rank);
  max_rank = std::max(max_rank, rank);
  return {std::move(Vi), transpose(Si_T)};
}
...
H2::generate_V_transfer_matrix(Matrix& Vbig_child1, Matrix& Vbig_child2, int64_t node,
  ...
  Matrix Vtransfer, R;
  std::tie(Vtransfer, R) = truncated_pivoted_qr(temp, accuracy);
  Matrix Si_T(R.rows, R.rows);
  Matrix Ui_T(R.rows, R.cols);
  rq(R, Si_T, Ui_T);
  ...
  min_rank = std::min(min_rank, rank);
  max_rank = std::max(max_rank, rank);
  return {std::move(Vtransfer), transpose(Si_T)};
}
...
void H2::update_column_cluster_bases(int64_t col, int64_t level,
  ...
  Matrix VN_col, R;
  std::tie(VN_col, R) = truncated_pivoted_qr(block_column_T, accuracy);
  Matrix SNT_col(R.rows, R.rows);
  Matrix UNT_col(R.rows, R.cols);
  rq(R, SNT_col, UNT_col);

  int64_t rank = VN_col.cols;
  min_rank = std::min(min_rank, rank);
  max_rank = std::max(max_rank, rank);

  Matrix t_col = matmul(VN_col, V(col, level), true, false);
  V.erase(col, level);
  V.insert(col, level, std::move(VN_col));

  Scol.erase(col, level);
  Scol.insert(col, level, transpose(SNT_col));
...
void H2::factorize_level(const Domain& domain,
  ...
  for (int64_t block = 0; block < nblocks; ++block) {
    if (block > 0) {
      ...
      // Check for fill-ins along row and column
      ...
      if (found_row_fill_in) {
        for (int64_t j = 0; j < nblocks; ++j) {
          if (is_admissible.exists(block, j, level) && is_admissible(block, j, level)) {
            ...
            if (F.exists(block, j)) {
              if (F(block, j).rows == block_size && F(block, j).cols == V(j, level).cols) {
    ...
    Matrix V_F = prepend_complement(V(block, level));
    ...
    int64_t row_split = D(block, block, level).rows - U(block, level).cols,
            col_split = D(block, block, level).cols - V(block, level).cols;
    ...
    // Schur's complement between cc blocks.
    for (int64_t i = block+1; i < nblocks; ++i) {
      for (int64_t j = block+1; j < nblocks; ++j) {
        if ((is_admissible.exists(block, j, level) && !is_admissible(block, j, level)) &&
    ...
    // Schur's complement between oc and co blocks.
    for (int64_t i = 0; i < nblocks; ++i) {
      for (int64_t j = 0; j < nblocks; ++j) {
        if ((is_admissible.exists(block, j, level) && !is_admissible(block, j, level)) &&
            (is_admissible.exists(i, block, level) && !is_admissible(i, block, level))) {
    ...
    // Schur's complement between cc and co blocks where the result exists
      ...
      for (int64_t j = 0; j < nblocks; ++j) {
        if ((is_admissible.exists(block, j, level) && !is_admissible(block, j, level)) &&
          ...
          // The product is a (co; oo)-sized matrix.
            ...
            // The Schur's complements that are formed before the block index are always
    ...
    // Schur's complement between oc and cc blocks where the result exists
    ...
    for (int64_t i = 0; i < nblocks; ++i) {
      for (int64_t j = block+1; j < nblocks; ++j) {
        if ((is_admissible.exists(block, j, level) && !is_admissible(block, j, level)) &&
          ...
          // Schur's complement between oc and cc blocks where a new fill-in is created.
...
void H2::factorize(const Domain& domain) {
  ...
  for (; level > 0; --level) {
    ...
    factorize_level(domain, level, nblocks, r, t);
    ...
    for (int64_t block = 0; parent_level > 0 && block < nblocks; block += 2) {
      ...
      if (col_has_admissible_blocks(parent_node, parent_level)) {
        ...
        Matrix temp(V(c1, level).cols + V(c2, level).cols, Vtransfer.cols);
        auto temp_splits = temp.split(vec{V(c1, level).cols}, vec{});
