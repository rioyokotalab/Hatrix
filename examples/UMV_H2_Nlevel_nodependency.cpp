...
class H2 {
 ...
 private:
  ...
  void update_row_cluster_bases(const int64_t level, const RowColLevelMap<Matrix>& F);
  void update_column_cluster_bases(const int64_t level, const RowColLevelMap<Matrix>& F);
...
void H2::update_row_cluster_bases(const int64_t level,
  ...
  for (int64_t i = 0; i < nblocks; i++) {  // Parallel OK
    ...
    if (fill_in.cols > 0) {  // Fill-in found
      ...
      // TODO Try to use dense block for LR using generate_block_row/column func for better accuracy
      Matrix block_row = U.exists(i, level) ?
                         concat(matmul(U(i, level), Srow(i, level)), fill_in, 1) :
                         fill_in;
      Matrix Ui, Si, Vi;
      std::tie(Ui, Si, Vi) = svd_like_compression(block_row);
      min_rank = std::min(min_rank, Ui.cols);
...
void H2::update_column_cluster_bases(const int64_t level,
                                     const RowColLevelMap<Matrix>& F) {
  const int64_t nblocks = level_blocks[level];
  RowMap<Matrix> t;
  for (int64_t j = 0; j < nblocks; j++) {  // Parallel OK
    ...
    if (fill_in.rows > 0) {  // Fill-in found
      ...
      Matrix block_column = V.exists(j, level) ?
                            concat(matmul(Scol(j, level), V(j, level), false, true), fill_in, 0) :
                            fill_in;
      Matrix block_column_T = transpose(block_column);
      Matrix Uj, Sj, Vj;
      std::tie(Uj, Sj, Vj) = svd_like_compression(block_column_T);
      min_rank = std::min(min_rank, Uj.cols);
      max_rank = std::max(max_rank, Uj.cols);

      if (V.exists(j, level)) {  // Not all dense column
        t.insert(j, matmul(Uj, V(j, level), true, false));
...
void H2::factorize() {
  ...
  for (; level > 0; --level) {
    ...
    update_row_cluster_bases(level, F);
    update_column_cluster_bases(level, F);
...
int main(int argc, char ** argv) {
  ...
  Hatrix::set_kernel_constants(1e-2 / (double)N, 1.);
