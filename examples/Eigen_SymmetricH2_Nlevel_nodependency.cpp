...
SymmetricH2::get_kth_eigenvalue(const Domain& domain,
  ...
  return {(a[idx_k] + b[idx_k]) / 2, max_rank_shift};
}
...
int main(int argc, char ** argv) {
  ...
  if (print_csv_header == 1) {
    ...
    std::cout << "N,leaf_size,accuracy,acc_type,max_rank,LRA,admis,matrix_type,kernel,geometry"
              ...
              << ",m,a0,b0,v_a0,v_b0,ev_tol,h2_eig_time,ldl_min_rank,ldl_max_rank,ldl_avg_rank,h2_eig_mem,max_rank_shift,dense_eigv,h2_eigv,eig_abs_err,success"
              << std::endl;
  }
#endif
  ...
  for (int64_t k = 0; k < target_m.size(); k++) {
    ...
    std::tie(h2_mth_eigv, max_rank_shift) =
    ...
    {
      Hatrix::SymmetricH2 M_shifted(M);
      const double lambda = max_rank_shift;
      // Shift leaf level diagonal blocks
      int64_t leaf_num_nodes = M_shifted.level_blocks[M_shifted.height];
      for(int64_t node = 0; node < leaf_num_nodes; node++) {
        shift_diag(M_shifted.D(node, node, M_shifted.height), -lambda);
      }
      M_shifted.factorize(domain);
      ldl_min_rank = M_shifted.get_basis_min_rank();
      ldl_max_rank = M_shifted.get_basis_max_rank();
      ldl_avg_rank = M_shifted.get_basis_avg_rank();
      h2_eig_mem = construct_mem + M_shifted.memory_usage();
#ifdef DEBUG_OUTPUT
      M_shifted.print_ranks();
#endif
    }
