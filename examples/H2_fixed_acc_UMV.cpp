...
void H2::update_row_cluster_bases(int64_t row, int64_t level,
  ...
  Matrix UN_row, SN_row, _VNT_row;
  std::tie(UN_row, SN_row, _VNT_row) = error_svd(block_row, accuracy);
...
void H2::update_column_cluster_bases(int64_t col, int64_t level,
  ...
  Matrix _UN_col, SN_col, VNT_col;
  std::tie(_UN_col, SN_col, VNT_col) = error_svd(block_column, accuracy);
  int64_t rank = VNT_col.rows;
  min_rank = std::min(min_rank, rank);
  max_rank = std::max(max_rank, rank);

  Matrix t_col = matmul(VNT_col, V(col, level));
  V.erase(col, level);
  V.insert(col, level, transpose(VNT_col));

  Scol.erase(col, level);
  Scol.insert(col, level, std::move(SN_col));
