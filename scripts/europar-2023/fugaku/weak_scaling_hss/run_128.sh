...
export PKG_CONFIG_PATH=$PKG_CONFIG_PATH:/vol0003/hp190122/u01594/gitrepos/parsec-profiling/build/lib64/pkgconfig
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/vol0003/hp190122/u01594/gitrepos/parsec-profiling/build/lib64
...
for adm in 7; do
    ...
    for nleaf in 512; do
        for max_rank in 25; do
    	    for N in 131072; do
                mpiexec -stdout out_128.log -stderr err_128.log ./bin/HSS_dtd --N $N \
               	       --nleaf $nleaf \
               	       --kernel_func laplace \
		...
		mkdir profiling_128
		mv hatrix_profile-* profiling_128/
    	    done
        done
    done
done
