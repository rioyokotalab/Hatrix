...
Matrix& RowMap::operator[](int64_t key) {
  return map.at(key);
}
const Matrix& RowMap::operator[](int64_t key) const {
  return map.at(key);
}
...
const Matrix& RowColMap::operator[](
  const std::tuple<int64_t, int64_t>& key
) const {
  return map.at(key);
}
...
void RowColMap::insert(
  const std::tuple<int64_t, int64_t> key, Matrix&& matrix
) {
  map.insert({key, std::move(matrix)});
}
