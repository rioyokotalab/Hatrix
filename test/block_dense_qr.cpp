...
void apply_block_householder(
  ...
  for(int i = 0; i < identity.rows; i++) identity(i, i) = 1.;
  Hatrix::matmul(
...
TEST(BlockDense, qr) {
  ...
  // Apply Q0^T to ( A01 ) = ( R01 )
  //               ( A11 )   ( A11 )
  apply_block_householder(A[0][0], A[1][0], T0, A[0][1], A[1][1], true);
  // QR(A11) = Q'11 * R11
  Hatrix::Matrix T1(block_size, block_size);
  Hatrix::geqrt(A[1][1], T1);
  ...
  // Apply Q'11 to Q11
  larfb(A[1][1], T1, Q[1][1], false);
  // Apply Q0 to ( Q00 ) and ( Q01 )
  //             ( Q10 )     ( Q11 )
  apply_block_householder(A[0][0], A[1][0], T0, Q[0][0], Q[1][0], false);
  apply_block_householder(A[0][0], A[1][0], T0, Q[0][1], Q[1][1], false);
  ...
  // Check accuracy
  for (int i=0; i<Dense_QR.rows; i++) {
    for (int j=0; j<Dense_QR.cols; j++) {
  ...
  for (int i=0; i<Dense_QTQ.rows; i++) {
    for (int j=0; j<Dense_QTQ.cols; j++) {
